...
namespace MagicaCloth
{
    ...
    public class MeshParticleWorker : PhysicsManagerWorker
    {
        ...
        private struct VertexToParticleJob : IJobParallelFor
        {
            ...
            [Unity.Collections.ReadOnly]
            public NativeParallelHashMap<int, int> vertexToParticleMap;
            ...
            public void Execute(int index)
            {
                ...
                int pindex;
                if (vertexToParticleMap.TryGetValue(vindex, out pindex))
                {
                    // 頂点の姿勢
        ...
        private struct ParticleToVertexJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                if (vertexToParticleMap.TryGetValue(vindex, out pindex))
                {
                    ...
                    float3 pos = ppos;
                    float3 nor = math.mul(prot, new float3(0, 0, 1));
                    float3 tan = math.mul(prot, new float3(0, 1, 0));
