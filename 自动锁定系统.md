...
允许玩家通过鼠标中键快速锁定和切换敌人目标，实现精确的第三人称战斗体验。
...
| 参数                    | 类型      | 默认值 | 说明                           |
| ----------------------- | --------- | ------ | ------------------------------ |
| `lockDetectionRadius` | float     | 30f    | 检测敌人目标的最大半径（米）   |
| `enemyLayerMask`      | LayerMask | -1     | 指定检测的层级，-1表示所有层级 |
...
| 参数                  | 类型  | 默认值 | 说明                               |
| --------------------- | ----- | ------ | ---------------------------------- |
| `lockRotationSpeed` | float | 10f    | 锁定模式下角色转向目标的速度       |
| `minLockDistance`   | float | 1f     | 最小锁定距离，防止锁定过近目标     |
| `maxLockDistance`   | float | 20f    | 最大锁定距离，超出范围自动取消锁定 |

### 目标要求

- **命名规范**: 目标对象名称必须包含"Enemy"（不区分大小写）
- **距离限制**: 目标必须在 `minLockDistance` 和 `maxLockDistance` 之间
- 后面记得改为敌人设置专门的层级以提高性能，**用名字区分太蠢了**
...
```
开始检测 → 获取检测半径内的碰撞体 → 筛选包含"Enemy"的对象 → 验证距离范围 → 按距离排序 → 更新可用目标列表
```

### 2. 锁定切流程

```
按中键 → 检查当前状态 → 开始锁定/切换目标/退出锁定 → 更新锁定状态 → 验证目标有效性
```

### 3. 实时验证

```
每帧检查 → 验证目标存在性 → 验证距离有效性 → 无效时自动退出锁定
```
...
- 根据游戏场景大小调整 `lockDetectionRadius`
- 避免设置过大的检测半径影响性能

### 3. 目标命名规范

- 统一使用包含"Enemy"的命名规范
- 避免在非敌人对象中使用"Enemy"关键词

### 4. 添加锁定UI指示器

```csharp
// 在锁定目标时显示UI指示器
public void ShowLockIndicator(Transform target)
{
    // 显示锁定UI逻辑
}
```
